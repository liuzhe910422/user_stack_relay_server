...
#define LOCAL_IP	"192.168.161.158"
#define LOCAL_MAC	"00:50:56:33:67:ba"
#define SERVER_IP	"192.168.161.160"
#define GATWAY_MAC	"00:50:56:f2:3d:0a"
...
void echo_udp_pkt_normal(struct udppkt *udp, struct udppkt *udp_rt) {
...
void relay_udp_pkt(struct udppkt *udp, struct udppkt *udp_rt) {
	...
	udp_rt->ip.saddr = inet_addr(SERVER_IP);
	udp_rt->ip.daddr = udp->ip.saddr;
...
void process_data_udp(struct nm_desc *nmr,struct udppkt *udp)
{
	struct in_addr addr;
	...
	relay_udp_pkt(udp, &udp_rt);
	//printf("send : %s\n",udp_rt.body);
	nm_inject(nmr, &udp_rt, sizeof(struct udppkt));
}
void process_data_icmp(struct nm_desc *nmr,struct udppkt *udp)
{
	struct icmppkt *icmp = (struct icmppkt*)udp;
...
void process_data_ip(struct nm_desc *nmr,unsigned char *stream)
{
	struct udppkt *udp = (struct udppkt*)stream;
	if (udp->ip.protocol == PROTO_UDP)
	{
		process_data_udp(nmr,udp);
	}else if(udp->ip.protocol == PROTO_ICMP)
	{
		process_data_icmp(nmr,udp);
	}
	else
	{
		//other like tcp etc.
...
void process_data_arp(struct nm_desc *nmr,unsigned char *stream)
{
	struct arppkt *arp = (struct arppkt *)stream;
	struct arppkt arp_rt;

	if (arp->arp.dip == inet_addr(LOCAL_IP)) {
		echo_arp_pkt(arp, &arp_rt,LOCAL_MAC);
		nm_inject(nmr, &arp_rt, sizeof(struct arppkt));
		printf("sent arp!\n");
	}
}

void process_data(struct nm_desc *nmr,unsigned char *stream)
{
	struct ethhdr *eh;
	eh = (struct ethhdr*)stream;
	if (ntohs(eh->h_proto) == PROTO_IP)
	{
		process_data_ip(nmr,stream);
		
	}else if(ntohs(eh->h_proto) == PROTO_ARP)
	{
		process_data_arp(nmr,stream);
	}
